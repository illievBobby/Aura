...
contract AuraToken {
    ...
    string public version = 'H1.4';       //human 0.1 standard. Just an arbitrary versioning scheme.
    ...
    constructor () public {
        ...
        rateBuy = 90000000000;              // 9 eth per AURA
        ///rateSell = 404000000;
    ...
    function printCoins(uint256 _amount) public {       // must be signed from all owners
    ...
    function withdraw(address owner, uint256 _amount) public {  // must be signed from 2 owners
        require(_amount > 0);
        require((msg.sender == owner1) || (msg.sender == owner2) || (msg.sender == owner3));
        if((owner != w_owner) && (_amount == w_amount)) {
            w_amount = 0;
